...
describe("isDueToSend", () => {
  ...
  it("sends when last_sent_at is before the due instant", () => {
    expect(isDueToSend(now, SUNDAY_20, jst(2026, 9, 6, 20, 1).toISOString())).toBe(
      true,
    );
  });

  it("does not send when last_sent_at equals the due instant", () => {
    expect(isDueToSend(now, SUNDAY_20, jst(2026, 9, 13, 20, 0).toISOString())).toBe(
      false,
    );
  });

  it("does not send when last_sent_at is after the due instant", () => {
    expect(isDueToSend(now, SUNDAY_20, jst(2026, 9, 13, 20, 1).toISOString())).toBe(
      false,
    );
  });
});
...
describe("windowEndingAt / isInstantInWindow", () => {
  ...
  it("includes a memo at the window start", () => {
    expect(isInstantInWindow(jst(2026, 9, 6, 20, 0).toISOString(), window)).toBe(
      true,
    );
  });

  it("excludes a memo at the due instant (next week)", () => {
    expect(isInstantInWindow(jst(2026, 9, 13, 20, 0).toISOString(), window)).toBe(
      false,
    );
  });

  it("drops a memo from an older week", () => {
    expect(isInstantInWindow(jst(2026, 9, 6, 19, 59).toISOString(), window)).toBe(
      false,
    );
  });
